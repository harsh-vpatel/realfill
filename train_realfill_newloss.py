...
def log_validation(
    ...
    target_image_path, target_mask_path = target_dir / "target.png", target_dir / "mask.png"
    
    # FIX 1: Name this init_image instead of image so we don't overwrite it
    init_image, mask_image = Image.open(target_image_path), Image.open(target_mask_path)
    ...
    for _ in range(args.num_validation_images):
        # FIX 2: Use init_image as the input, save the output to gen_image
        # FIX 3: Lowered steps to 50 for speed, increased guidance_scale to 7.5 for clarity
        gen_image = pipeline(
            prompt="a photo of sks", 
            image=init_image, 
    ...
    for tracker in accelerator.trackers:
        ...
        if tracker.name == "wandb":
            tracker.log(
                {
                    f"validation": [
                        wandb.Image(img, caption=str(i)) for i, img in enumerate(images)
                    ]
                }
            )
...
def parse_args(input_args=None):
    ...
    # --- MASK LOSS MULTIPLIER ADDITION ---
    parser.add_argument(
        "--mask_loss_multiplier",
        type=float,
        default=5.0,
        help="Multiplier for the loss inside the masked regions to force the model to focus on inpainting. Default 5.0.",
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet, text_encoder):
                ...
                # Calculate the raw, pixel-by-pixel loss
                raw_loss = F.mse_loss(model_pred.float(), noise.float(), reduction="none")

                # Apply multiplier to the synthetic holes in reference images.
                # 'masks' == 1.0 inside the holes, 0.0 in the background.
                # 'weightings' == 1.0 where we have valid ground truth (target bg, and all ref pixels).
                # The target hole has weightings == 0, so its loss remains 0.
                custom_weights = weightings * (1.0 + (args.mask_loss_multiplier - 1.0) * masks)
                
                # Take the mean over the weighted raw loss
                loss = (custom_weights * raw_loss).mean()
                # -------------------------------------------------------------
